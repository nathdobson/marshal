...
pub fn deserialize_arc< D: Decoder, T: 'static + Sync + Send + Deserialize< D>>(
    d: AnyDecoder<'_,  D>,
    mut ctx: Context,
...
pub fn deserialize_rc< D: Decoder, T: 'static + Deserialize< D>>(
    d: AnyDecoder<'_,  D>,
    mut ctx: Context,
) -> anyhow::Result<Rc<T>> {
    let shared = <Shared<T> as Deserialize< D>>::deserialize(d, ctx.reborrow())?;
...
pub fn deserialize_rc_weak< D: Decoder, T: 'static + Deserialize< D>>(
    d: AnyDecoder<'_,  D>,
    mut ctx: Context,
...
macro_rules! derive_deserialize_rc_shared {
    ($ty:ty) => {
        ...
        {
            fn deserialize_rc<'p>(
                p: $crate::reexports::marshal::decode::AnyDecoder<'p,  D>,
