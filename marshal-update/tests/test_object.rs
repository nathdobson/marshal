...
use marshal_derive::{Deserialize, Serialize, SerializeStream, SerializeUpdate};
...
pub trait Foo:
    'static + Debug + RawAny + AsDiscriminant<BoxFoo> + SerializeUpdateDyn<JsonEncoder>
{
}

impl SerializeStream for Box<dyn Foo> {
    type Stream = Box<dyn Sync + Send + Any>;
...
impl DeserializeUpdate<JsonDecoder> for Box<dyn Foo>{
    fn deserialize_update<'p, 'de>(&mut self, d: AnyDecoder<'p, 'de, JsonDecoder>, ctx: Context) -> anyhow::Result<()> {
        todo!()
    }
}
...
#[derive(
    Serialize, SerializeUpdate, SerializeStream, Deserialize, Debug, Eq, PartialEq, Ord, PartialOrd,
)]
struct A(u8);
impl Foo for A {}
