...
pub trait DeserializeArc<D: Decoder> {
    fn deserialize_arc<'p>(p: AnyDecoder<'p, D>, ctx: Context) -> anyhow::Result<Arc<Self>>;
}
...
pub trait DeserializeRc<D: Decoder> {
    fn deserialize_rc<'p>(p: AnyDecoder<'p, D>, ctx: Context) -> anyhow::Result<Rc<Self>>;
}
