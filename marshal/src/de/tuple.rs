    ...
    fn deserialize(p: P::AnyDecoder<'_>, context: &mut Context) -> anyhow::Result<Self> {
        match p.decode(DecodeHint::Tuple { len: 1 })? {
            DecoderView::Seq(mut p) => {
                let f0 = T1::deserialize(p.decode_next()?.ok_or(SchemaError::TupleTooShort)?, context)?;
    ...
    for (T1, T2)
{
    fn deserialize(p: P::AnyDecoder<'_>, context: &mut Context) -> anyhow::Result<Self> {
        match p.decode(DecodeHint::Tuple { len: 2})? {
            DecoderView::Seq(mut p) => {
                let f0 = T1::deserialize(p.decode_next()?.ok_or(SchemaError::TupleTooShort)?, context)?;
                let f1 = T2::deserialize(p.decode_next()?.ok_or(SchemaError::TupleTooShort)?, context)?;
                if let Some(_) = p.decode_next()? {
                    return Err(SchemaError::TupleTooLong.into());
                }
                Ok((f0, f1))
    ...
    fn deserialize(p: P::AnyDecoder<'_>, context: &mut Context) -> anyhow::Result<Self> {
        match p.decode(DecodeHint::Tuple { len: 3 })? {
            DecoderView::Seq(mut p) => {
                let f0 = T1::deserialize(p.decode_next()?.ok_or(SchemaError::TupleTooShort)?, context)?;
                let f1 = T2::deserialize(p.decode_next()?.ok_or(SchemaError::TupleTooShort)?, context)?;
                let f2 = T3::deserialize(p.decode_next()?.ok_or(SchemaError::TupleTooShort)?, context)?;
                if let Some(_) = p.decode_next()? {
                    return Err(SchemaError::TupleTooLong.into());
                }
                Ok((f0, f1, f2))
    ...
    fn deserialize(p: P::AnyDecoder<'_>, context: &mut Context) -> anyhow::Result<Self> {
        match p.decode(DecodeHint::Tuple { len: 4 })? {
            DecoderView::Seq(mut p) => {
                ...
                let f1 = T2::deserialize(p.decode_next()?.ok_or(SchemaError::TupleTooShort)?, context)?;
                let f2 = T3::deserialize(p.decode_next()?.ok_or(SchemaError::TupleTooShort)?, context)?;
                let f3 = T4::deserialize(p.decode_next()?.ok_or(SchemaError::TupleTooShort)?, context)?;
                if let Some(_) = p.decode_next()? {
                    return Err(SchemaError::TupleTooLong.into());
                }
                Ok((f0, f1, f2, f3))
