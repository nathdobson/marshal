...
macro_rules! weak_ref {
    ($strong_ref:ident, $weak_ref:ident, $strong:ident, $weak:ident) => {
        #[repr(transparent)]
        pub struct $weak_ref<T: ?Sized> {
            phantom: PhantomData<*const ()>,
            inner: UnsafeCell<T>,
        }

        impl<T: ?Sized> AsFlatRef for $weak<T> {
...
weak_ref!(RcRef, RcWeakRef, Rc, RcWeak);
weak_ref!(ArcRef, ArcWeakRef, ArcRef, ArcWeak);
